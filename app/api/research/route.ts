...
const ResearchSchema = z.object({
  ...
  keyPeople: z
    ...
    .describe("Names of founders, executives, or team leads mentioned on the site"),
});
...
type Contact = { name: string; email: string; position: string; verified: boolean };
...
function scoreEmail(email: HunterEmail, targetRole: string) {
  ...
  if (email.verification?.status === "valid" || email.verification?.status === "verified") score += 2;

  // Slight penalty for generic emails but don't exclude them
  const genericPatterns = [/^info@/i, /^support@/i, /^help@/i, /^hello@/i, /^contact@/i, /^sales@/i];
...
async function verifyEmail(email: string, apiKey: string): Promise<{ valid: boolean; status: string; score: number }> {
  try {
    ...
    console.log("[Hunter Email Verifier] Response:", JSON.stringify(data?.data, null, 2));
...
async function hunterEmailFinder(
  ...
  try {
    ...
    console.log("[Hunter Email Finder] Trying:", firstName, lastName, "@", domain);
    const res = await fetch(url);
    const data = await res.json();
    console.log("[Hunter Email Finder] Response:", JSON.stringify(data, null, 2));
    if (data?.data?.email) {
  ...
  try {
    ...
    for (const path of altPaths) {
      try {
        ...
        const alt = await firecrawl.scrapeUrl(altUrl, { formats: ["markdown"] });
    ...
    const combinedMarkdown = [scrapeResult.markdown, ...careerMarkdown].join("\n\n").slice(0, 20000);

    const { object: intel } = await generateObject({
      model: openai("gpt-4o-mini"),
      schema: ResearchSchema,
      prompt: `You are a research assistant helping a CS student tailor outreach.
...
${combinedMarkdown}`,
    ...
    console.log("[Research] LLM extracted intel:", JSON.stringify(intel, null, 2));
    ...
    if (process.env.HUNTER_API_KEY) {
      ...
      try {
        ...
        console.log("[Hunter Domain Search] URL:", hunterUrl.replace(process.env.HUNTER_API_KEY!, "***"));
        const hunterRes = await fetch(hunterUrl);
        const hunterData = await hunterRes.json();
        console.log("[Hunter Domain Search] Status:", hunterRes.status);
        console.log("[Hunter Domain Search] Response meta:", JSON.stringify(hunterData?.meta, null, 2));
        console.log("[Hunter Domain Search] Email count:", hunterData?.data?.emails?.length ?? 0);

        const emails: HunterEmail[] = hunterData?.data?.emails ?? [];

        if (emails.length > 0) {
          console.log("[Hunter Domain Search] Scoring", emails.length, "emails...");
          const ranked = emails
            .map((email) => ({
              email,
          ...
          ranked.slice(0, 3).forEach((r, i) => {
            console.log(`  ${i + 1}. ${r.email.value} (${r.email.position || "no position"}) - score: ${r.score}`);
          });

          // Try to verify the top ranked emails until we find a valid one
          for (const { email: best } of ranked.slice(0, 5)) {
            if (best && best.value) {
              const verification = await verifyEmail(best.value, process.env.HUNTER_API_KEY!);
              console.log("[Hunter Domain Search] Email verification for", best.value, ":", verification.status, "score:", verification.score);
              ...
              if (verification.valid) {
                console.log("[Hunter Domain Search] Found verified email:", JSON.stringify(contact, null, 2));
                break;
              } else {
                console.log("[Hunter Domain Search] Email not verified, trying next...");
              }
            }
          }
          
          if (contact) {
            console.log("[Hunter Domain Search] Selected contact:", JSON.stringify(contact, null, 2));
          }
        } else {
      ...
      if (!contact && intel.keyPeople && intel.keyPeople.length > 0) {
        console.log("[Research] Step 4: Trying Hunter.io Email Finder with", intel.keyPeople.length, "key people...");
        for (const person of intel.keyPeople.slice(0, 3)) {
          if (person.firstName && person.lastName) {
            const found = await hunterEmailFinder(domain, person.firstName, person.lastName, process.env.HUNTER_API_KEY!);
            if (found && found.email) {
              // Verify the found email
              const verification = await verifyEmail(found.email, process.env.HUNTER_API_KEY!);
              console.log("[Hunter Email Finder] Email verification for", found.email, ":", verification.status, "score:", verification.score);
              ...
              console.log("[Hunter Email Finder] Found email:", JSON.stringify(contact, null, 2));
    ...
    console.log("[Research] Complete. Contact:", contact ? "YES" : "NO", "| Verified:", contact?.verified ? "YES" : "NO");
